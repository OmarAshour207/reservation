            ...
            <div class="card card-form__body card-body">
                <form method="post" action="{{ route('accounts.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input id="email" name="email" type="text" class="form-control" placeholder="{{ __('admin.email') }}" value="{{ old('email') }}">
                    </div>

                    <div class="form-group">
                        <label for="password"> {{ trans('admin.accounts') }} / {{ trans('admin.password') }}</label>
                        <input id="password" name="password" type="password" class="form-control" placeholder="{{ __('admin.password') }}">
                    </div>

                    <div class="form-group">
                        <label for="role"> {{ trans('admin.accounts') }} / {{ trans('admin.choose_role') }}</label> <br>
                        <select id="role" name="role" data-toggle="select" class="form-control select2">
                            <option value="" selected> {{ __('admin.choose_role') }} </option>
                            <option value="1"> {{ __('admin.doctor') }} </option>
                            <option value="2"> {{ __('admin.nursery') }} </option>
                        </select>
                    </div>
